...
static bool ggml_cuda_should_fuse_mul_mat(const ggml_tensor * ffn_up,
    ...
    static constexpr std::array<ggml_glu_op, 4> valid_glu_ops = { GGML_GLU_OP_SWIGLU, GGML_GLU_OP_GEGLU, GGML_GLU_OP_SWIGLU_OAI, GGML_GLU_OP_SITU };
    ...
    switch (op->op) {
        ...
        case GGML_OP_GLU:
            switch (ggml_get_glu_op(op)) {
                ...
                case GGML_GLU_OP_SITU:
                    return op->type == GGML_TYPE_F32 && ggml_is_contiguous_1(op->src[0]) &&
                        (!op->src[1] || ggml_is_contiguous_1(op->src[1]));
                default:
                    return false;
            }
            break;
