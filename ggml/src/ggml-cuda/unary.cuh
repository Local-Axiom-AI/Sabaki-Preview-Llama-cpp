...
__device__ __forceinline__ float ggml_cuda_op_situ_single(float gate, float up) {
    // Keep the rounding boundaries of the original multi-kernel expression.
    const float bounded_gate = __fmul_rn(tanhf(__fmul_rn(0.25f, gate)), 4.0f);
    const float gate_act = __fmul_rn(bounded_gate, 1.0f / (1.0f + expf(-gate)));
    const float bounded_up = __fmul_rn(tanhf(__fmul_rn(0.04f, up)), 25.0f);
    return __fmul_rn(gate_act, bounded_up);
}
