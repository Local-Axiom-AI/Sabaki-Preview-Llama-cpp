...
static __global__ void situ_kernel(const float * gate, const float * up, float * dst,
                                  int64_t count, int64_t width, int64_t gate_stride, int64_t up_stride) {
    const int64_t index = (int64_t) blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= count) return;
    const int64_t row = index / width;
    const int64_t col = index % width;
    dst[index] = ggml_cuda_op_situ_single(gate[row * gate_stride + col], up[row * up_stride + col]);
}

void ggml_cuda_op_situ(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const auto * gate = dst->src[0];
    const auto * up = dst->src[1] ? dst->src[1] : gate;
    GGML_ASSERT(gate->type == GGML_TYPE_F32 && up->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous_1(gate) && ggml_is_contiguous_1(up) && ggml_is_contiguous(dst));
    const int64_t width = dst->ne[0];
    const bool packed = dst->src[1] == nullptr;
    const bool swapped = ggml_get_op_params_i32(dst, 1) != 0;
    const float * g = (const float *) gate->data + (packed && swapped ? width : 0);
    const float * u = (const float *) up->data + (packed && !swapped ? width : 0);
    const int64_t count = ggml_nelements(dst);
    situ_kernel<<<(count + CUDA_GLU_BLOCK_SIZE - 1) / CUDA_GLU_BLOCK_SIZE, CUDA_GLU_BLOCK_SIZE, 0, ctx.stream()>>>(
        g, u, (float *) dst->data, count, width, gate->nb[1] / sizeof(float), up->nb[1] / sizeof(float));
}
