...
static void ggml_compute_forward_situ(const ggml_compute_params * params, ggml_tensor * dst) {
    ...
    for (int64_t row = params->ith; row < ggml_nrows(dst); row += params->nth) {
        const auto * g = (const float *) ((const char *) gate->data + row * gate->nb[1]) + (packed && swapped ? width : 0);
        const auto * u = (const float *) ((const char *) up->data + row * up->nb[1]) + (packed && !swapped ? width : 0);
        auto * out = (float *) ((char *) dst->data + row * dst->nb[1]);
        for (int64_t col = 0; col < width; ++col) {
            const float bounded_gate = 4.0f * tanhf(0.25f * g[col]);
            const float gate_act = bounded_gate * (1.0f / (1.0f + expf(-g[col])));
            const float bounded_up = 25.0f * tanhf(0.04f * u[col]);
            out[col] = gate_act * bounded_up;
        }
    }
}
